...
class MixDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        while True:
            ...
            if index < self.n_normal:
                # normal
                img1, target = self.normal_item()
            elif index < self.n_normal + self.n_mix:
                # mix
                img1, img2, target= self.mix_item()
            else:
                # poison
                img1, img2, target = self.poison_item()

            if img2 is not None:
                img3 = self.mixer.mix(img1, img2)
                if img3 is None:
    ...
    def mix_item(self):
        ...
        img1,_= self.basic_item(index1)
        # (img2, classK)
        index2 = self.random_choice(self.uni_index[classK])
        img2,_= self.basic_item(index2)
        return img1, img2, classK
